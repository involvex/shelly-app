...
export function MoreMenu({children}: {children?: React.ReactElement}) {
	...
	const page = project.pages.find(
		(page: PageContext) => page.id === project.selectedPage,
	)
	return (
		<Menu.Root>
			<Menu.Trigger className="flex justify-center outline-none align-center">
				{children}
			</Menu.Trigger>
...
export function UISettingsMenu({children}: {children?: React.ReactElement}) {
			...
			<Menu.Content>
				...
				<Menu.Group horizontal>
					{TOGGLES.map((toggle, index) => {
						const isSelected = index === toggleIndex
	...
	return (
		<Menu.Root>
			...
			<Menu.Content>
				...
				<Menu.Group>
					{project.pages.map((page: PageContext, index: number) =>
...
export function SortMenu({children}: {children: React.ReactElement}) {
	...
	return (
		<Menu.Root>
			<Menu.Trigger
				asChild={Platform.OS !== 'web'}
				className="flex justify-center outline-none align-center"
			>
				{children}
			</Menu.Trigger>
			...
			<Menu.Content>
				{SORT_FILTERS.map(({value, label}) => (
					<Menu.CheckboxItem
						key={value}
						value={value === sort}
						onValueChange={(next: string) => next === 'on' && setSort(value)}
					>
